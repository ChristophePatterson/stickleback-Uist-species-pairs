...
### sbatch /gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/2_Results/2_2_Divergence_estimates/15.20-calculate-css-drop-populations.sh "CLAC"
### sbatch /gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/2_Results/2_2_Divergence_estimates/15.20-calculate-css-drop-populations.sh "DUIN"
### sbatch /gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/2_Results/2_2_Divergence_estimates/15.20-calculate-css-drop-populations.sh "LUIB"
### sbatch /gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/2_Results/2_2_Divergence_estimates/15.20-calculate-css-drop-populations.sh "OBSE"
...
output_prefix=stickleback.n$pop.wnd$wndsize.sld$sliding.mnSNP$mnSNP.mth$wdnmthd-$mthd.MAF$MAF
output_dir=$wkdir/results/$vcf_ver/sliding-window/CSS/dropPops/n$pop/$output_prefix
# Create directory
mkdir -p $output_dir
...
if [ $SLURM_ARRAY_TASK_ID == "1" ]; then
   ...
   echo -e "CLAC\nOBSE\nDUIN\nLUIB" | grep -v $pop > $output_dir/Pops_interest.txt
...
if [ $permfilesNo == 21 ]; then
   ...
   Rscript /gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/2_Results/2_2_Divergence_estimates/15.01-plot-css.R \
      $output_dir ${output_prefix}.CSSm.10000perm.txt &> $output_dir/CSS_combined_log.txt
else
