...
## Summary window statistics
window_stats <- twisst_data_all %>% 
  group_by(run_name) %>%
  summarise(mn_length = mean(end-start, na.rm = T), 
            min_length = min(end-start, na.rm = T),
            max_length = max(end-start, na.rm = T),
            sd_length = sd(end-start, na.rm = T),
            n_wnd = length(start))
