...
SNP.library.name <- basename(gsub(".reheader.recode", "", plink.file))
SNP.library.path <- gsub(basename(plink.file), "", plink.file,)

## Set up SambaR
dir.create(paste0(plot.dir,"/",SNP.library.name))
setwd(paste0(plot.dir,"/",SNP.library.name))
source("/gpfs01/home/mbzcp2/R/x86_64-pc-linux-gnu-library/4.2/SambaR-master/SAMBAR_v1.10.txt")
## Install packages
getpackages()

## Read in data set 
importdata(inputprefix=paste0(plink.file),
             samplefile=paste0(SNP.library.path, "/pop_file_Population.txt"))
