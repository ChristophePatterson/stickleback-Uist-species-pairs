...
# Genome annoataions
fGas.bed <- as_tibble(read.table("/gpfs01/home/mbzcp2/data/sticklebacks/genomes/GCA_046562415.1_Duke_GAcu_1.0_genomic_functional_annotation/GCA_046562415.1_Duke_GAcu_1.0_genomic_blast_matches_genesOnly.gtf", 
                                  header = T))
fGas.bed$chr <- chr$Sequence.name[match(fGas.bed$chr, chr$GenBank.seq.accession)]
fGas.bed$start.cum <- fGas.bed$start+(chr$Cum.Seq.length[match(fGas.bed$chr, chr$Sequence.name)]+chr$Cum.Seq.length[1])
fGas.bed$end.cum <- fGas.bed$end+(chr$Cum.Seq.length[match(fGas.bed$chr, chr$Sequence.name)]+chr$Cum.Seq.length[1])

# Filter to only those that match
fGas.bed.contains <- fGas.bed[apply(fGas.bed, MARGIN = 1, function(x) any(grepl(x["fGas.name.match"], 
                                c(CSS.annotations$contains.genes.fGas[CSS.annotations$mn.CSS>2], CSS.annotations$contains.genes.fGas.in10Kbps[CSS.annotations$mn.CSS>2])), na.rm = T)),]
fGas.bed.contains$fGas.name.match <- gsub("LOC144383728", "myh7bb", fGas.bed.contains$fGas.name.match)
fGas.bed.contains$fGas.name.match <- gsub("LOC120817547", "msx2a", fGas.bed.contains$fGas.name.match)
fGas.bed.contains <- fGas.bed.contains[!grepl("LOC", fGas.bed.contains$fGas.name.match),]
...
CSS.annotations.top.regions$start.cum <- CSS.annotations.top.regions$start+(chr$Cum.Seq.length[match(CSS.annotations.top.regions$chr, chr$Sequence.name)]+chr$Cum.Seq.length[1])
CSS.annotations.top.regions$end.cum <- CSS.annotations.top.regions$end+(chr$Cum.Seq.length[match(CSS.annotations.top.regions$chr, chr$Sequence.name)]+chr$Cum.Seq.length[1])

# CSS.annotations.top.regions <- data.frame(chr = factor(c("I", "IV", "XI", "XXI"), levels = levels(chr$Sequence.name)), start = c(25000000, 12000000, 5000000, 8000000), end = c(31000000, 16000000, 10000000, 15000000))
regions <- data.frame(chr = factor(c("I", "IV", "XI", "XIV", "XIX"), levels = levels(chr$Sequence.name)),
                      start = c(26300000, 11500000, 5500000, 12500000, 2720000), 
                      end = c(27300000, 28500000, 7000000, 13500000, 2910000)) %>%
                      mutate(chr_group = paste0(chr, "_", 1:length(chr)))
...
inversions <- read.table("/gpfs01/home/mbzcp2/data/sticklebacks/genomes/Prior_gasAcu-results/Venu-et-al-2022-SupTab7-inversions-DUKE.bed") %>%
                    rename(chr = V1, start = V2, end = V3, inversion = V4, group = V5) %>%
                    mutate(chr = factor(as.character(as.roman(as.numeric(gsub("chr", "", chr))))))

inversions$start.cum <- inversions$start+(chr$Cum.Seq.length[match(inversions$chr, chr$Sequence.name)]+chr$Cum.Seq.length[1])
inversions$end.cum <- inversions$end+(chr$Cum.Seq.length[match(inversions$chr, chr$Sequence.name)]+chr$Cum.Seq.length[1])
...
# Filer dataset to specific region
fGas.bed.contains.filt <- fGas.bed.contains %>%
  rowwise() %>%
  filter(any(
    chr == regions$chr &
      start >= regions$start &
...
p.CSS <- ggplot(CSS.HQ[!CSS.HQ$drop.all.sig.qvalue.0001,]) +
  geom_vline(xintercept = chr$Cum.Seq.length, col = "grey80") +
  # geom_point(data = inversions[duplicated(inversions$inversion),], aes(x = start.cum, y = -max(CSS.HQ$css)*0.1), shape = 8, col = "grey60", size = 2) +
  ...
  geom_point(aes(start.cum, css, fill = as.factor(bi.col)), show.legend = F, shape = 21, stroke = 0) +
  geom_point(data = CSS.HQ[CSS.HQ$drop.all.sig.qvalue.0001,], aes(start.cum, css), shape = 21, , , stroke = 0, fill = "firebrick3") +
  ## geom_segment(data = CSS.annotations.top.regions, 
  ##       aes(x = start.cum, xend = end.cum, y = -1.5, col = "This Study"), linewidth = 2, lineend = "round") +
  geom_segment(data = jones_2012, 
        aes(x = start.cum, xend = end.cum, y = -max(CSS.HQ$css)*0.1, col = "Jones et al 2012"), linewidth = 2, lineend = "round") +
  geom_segment(data = Roberts_2021, 
      aes(x = start.cum, xend = end.cum, y = -max(CSS.HQ$css)*0.15, col = "Roberts et al 2021 - EcoPeaks"), linewidth = 2, lineend = "round") +
  geom_segment(data = inversions, 
      aes(x = start.cum, xend = end.cum, y = -max(CSS.HQ$css)*0.2, col = "Inversions - Venu et al 2024"), linewidth = 2, lineend = "round") +
  scale_fill_manual(values = c("black", "grey50")) +
  theme_bw() +
  ...
  scale_colour_manual(name = 'Significant regions', 
        values =c('This Study'='firebrick3','Jones et al 2012'='deepskyblue', 'Roberts et al 2021 - EcoPeaks'='orange', 'Inversions - Venu et al 2024' = 'grey60'),
        labels('This Study','Jones et al 2012','Roberts et al 2021 - EcoPeaks', 'Inversions - Venu et al 2024')) +
  theme(panel.spacing = unit(0,'lines'),
        panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(),
        axis.line = element_line(), strip.background = element_rect(color = "black", fill = "white", linewidth = 1),
...
# Save
# ggsave("/gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/test.png", p.CSS , height = 15.92*0.3333, width = 15.92*0.66666)

## Read in significant windows
CSS.annotations.top.regions.filt$genes.fGas <- gsub("\\|", ",", CSS.annotations.top.regions.filt$contains.genes.fGas)
CSS.annotations.top.regions.filt$genes.fGas <- gsub("LOC144383728", "myh7bb", CSS.annotations.top.regions.filt$genes.fGas)
CSS.annotations.top.regions.filt$genes.fGas <- gsub("LOC120817547", "msx2a", CSS.annotations.top.regions.filt$genes.fGas)

# Relabel LOC144383728 as myh7bb and remove all other LOC genes
...
tmp.dt <- pack_intervals(fGas.bed.contains.filt,
                           "start", "end", 100000 ,"track")

p.CSS.filt <- ggplot(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]) +
  geom_segment(data = tmp.dt,
          aes(x = start+((end-start)/2), y = 0, yend = max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)+(track*2)), col = "grey60") +
  ...
  geom_text(data = tmp.dt,
          aes(x = start+((end-start)/2), y = max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)+(track*2), label = fGas.name.match ), size = 4, vjust = -0.5, hjust = 0.5, col = "grey10") +
  geom_segment(data = CSS.annotations.top.regions.filt, 
        aes(x = start, xend = end, y = -1.5, col = "This Study"), linewidth = 2, lineend = "round") +
  geom_segment(data = jones_2012.filt, 
        aes(x = start, xend = end, y = -2.5, col = "Jones et al 2012"), linewidth = 2, lineend = "round") +
  geom_segment(data = Roberts_2021.filt, 
      aes(x = start, xend = end, y = -3.5, col = "Roberts et al 2021 - EcoPeaks"), linewidth = 2, lineend = "round") +
  geom_segment(data = inversions.filt, 
      aes(x = start, xend = end, y = -4.5, col = "Inversions - Venu et al 2024"), linewidth = 2, lineend = "round") +
  # scale_color_manual(values = c("black", "grey50")) +
  scale_fill_manual(values = c("black", "grey50")) +
  facet_wrap(~chr, scale = "free_x") +
  theme_bw() +
  scale_x_continuous(labels = function(x) paste0(x / 1e6),name = "Mbps", expand = c(0,0)) +
  scale_y_continuous(name = "CSS", expand = c(0.1,0.1)) +
  scale_colour_manual(name = 'Significant regions', 
         values =c('This Study'='firebrick3','Jones et al 2012'='deepskyblue', 'Roberts et al 2021 - EcoPeaks'='orange', 'Inversions - Venu et al 2024' = 'grey60'),
         labels('This Study','Jones et al 2012','Roberts et al 2021 - EcoPeaks', 'Inversions - Venu et al 2024')) +
...
p.CSS.n.chr4 <- ggplot(CSS.HQ.filt[(!CSS.HQ.filt$drop.all.sig.qvalue.0001)&CSS.HQ.filt$chr!="IV",]) +
  geom_segment(data = tmp.dt[tmp.dt$chr!="IV",],
          aes(x = start+((end-start)/2), y = 0, yend = (max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)*1.2)+(track*2)), col = "grey60") +
  geom_point(aes(start, css), col = "black", show.legend = F) +
  geom_point(data = CSS.HQ.filt[CSS.HQ.filt$drop.all.sig.qvalue.0001&CSS.HQ.filt$chr!="IV",], aes(start, css), col = "firebrick3") +
  ## geom_text(data = CSS.annotations.top.regions.filt[CSS.annotations.top.regions.filt$mn.CSS>=2,], 
  ##         aes(x = start+((end-start)/2), y = mn.CSS, label = gsub(", ", "\n", genes.filt)), size = 0.5) +
  ## geom_text_repel(data = CSS.annotations.top.regions.filt[CSS.annotations.top.regions.filt$mn.CSS>=2&!duplicated(CSS.annotations.top.regions.filt$genes.filt),], 
  ##         aes(x = start+((end-start)/2), y = mn.CSS, label = gsub(", ", "\n", genes.filt)), hjust = 0, nudge_y = 5, nudge_x = 250000,
  ##         direction = "both", box.padding = 0.1, col = "grey10", segment.color = 'grey50',
  ##         size = 1.5, max.overlaps = 5, min.segment.length = 0, force = 10) +
  geom_text(data = tmp.dt[tmp.dt$chr!="IV",],
          aes(x = start+((end-start)/2), y = (max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)*1.2)+(track*2), label = fGas.name.match ), size = 4, vjust = -0.5, hjust = 0.5, col = "grey10") +
  geom_segment(data = CSS.annotations.top.regions.filt[CSS.annotations.top.regions.filt$chr!="IV",], 
        aes(x = start, xend = end, y = -1.5, col = "This Study"), linewidth = 2, lineend = "round") +
  geom_segment(data = jones_2012.filt[jones_2012.filt$chr!="IV",], 
        aes(x = start, xend = end, y = -2.5, col = "Jones et al 2012"), linewidth = 2, lineend = "round") +
  geom_segment(data = Roberts_2021.filt[Roberts_2021.filt$chr!="IV",], 
      aes(x = start, xend = end, y = -3.5, col = "Roberts et al 2021 - EcoPeaks"), linewidth = 2, lineend = "round") +
  geom_segment(data = inversions.filt[inversions.filt$chr!="IV",], 
      aes(x = start, xend = end, y = -4.5, col = "Inversions - Venu et al 2024"), linewidth = 2, lineend = "round") +
  # scale_color_manual(values = c("black", "grey50")) +
  scale_fill_manual(values = c("black", "grey50")) +
  facet_wrap(~chr, scale = "free_x") +
...
p.CSS.chr4 <- ggplot(CSS.HQ.filt[(!CSS.HQ.filt$drop.all.sig.qvalue.0001)&CSS.HQ.filt$chr=="IV",]) +
  geom_segment(data = tmp.dt[tmp.dt$chr=="IV",],
          aes(x = start+((end-start)/2), y = 0, yend = (max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)*1.2)+(track*2)), col = "grey60") +
  geom_point(aes(start, css), col = "black", show.legend = F) +
  geom_point(data = CSS.HQ.filt[CSS.HQ.filt$drop.all.sig.qvalue.0001&CSS.HQ.filt$chr=="IV",], aes(start, css), col = "firebrick3") +
  ...
  ##         size = 1.5, max.overlaps = 5, min.segment.length = 0, force = 10) +
  geom_text(data = tmp.dt[tmp.dt$chr=="IV",],
          aes(x = start+((end-start)/2), y = (max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)*1.2)+(track*2), label = fGas.name.match ), 
          size = 4, vjust = -0.5, hjust = 0.5, col = "grey10") +
  geom_segment(data = CSS.annotations.top.regions.filt[CSS.annotations.top.regions.filt$chr=="IV",], 
        aes(x = start, xend = end, y = -0.5, col = "This Study"), linewidth = 2, lineend = "round") +
  geom_segment(data = jones_2012.filt[jones_2012.filt$chr=="IV",], 
  ...
  geom_segment(data = inversions.filt[inversions.filt$chr=="IV",], 
      aes(x = start, xend = end, y = -2, col = "Inversions - Venu et al 2024"), linewidth = 2, lineend = "round") +
  # scale_color_manual(values = c("black", "grey50")) +
  scale_fill_manual(values = c("black", "grey50")) +
  facet_wrap(~chr, scale = "free_x") +
  theme_bw() +
  scale_x_continuous(labels = function(x) paste0(x / 1e6),name = "Mbps", expand = c(0,0)) +
  scale_y_continuous(name = "CSS", expand = c(0.1,0.1), limits = c(-2, max(CSS.HQ.filt[!CSS.HQ.filt$drop.all.sig.qvalue.0001,]$css)*1.2 + max(tmp.dt$track*2))) +
  scale_colour_manual(name = 'Significant regions', 
         values =c('This Study'='firebrick3','Jones et al 2012'='deepskyblue', 'Roberts et al 2021 - EcoPeaks'='orange', 'Inversions - Venu et al 2024' = 'grey60'),
         labels('This Study','Jones et al 2012','Roberts et al 2021 - EcoPeaks', 'Inversions - Venu et al 2024')) +
  theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(),
