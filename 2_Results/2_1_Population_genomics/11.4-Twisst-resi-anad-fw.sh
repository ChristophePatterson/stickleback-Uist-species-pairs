...
grep -f $wkdir/vcfs/$vcf_ver/${species}_samples.txt /gpfs01/home/mbzcp2/code/Github/stickleback-Uist-species-pairs/bigdata_Christophe_2025-04-28.csv | 
    awk -F ',' -v OFS='\t' '$13!="st" { print $1, $9, $10, $13 } $13=="st" { print $1, $9, $10, "fw" }' | \
    awk '$3 == "TORM" || $3 == "CLAC" || $3 == "DUIM" || $3 == "LUIB"' > $output_dir/pop_file_${run_name}.txt

## Add in new phased sample names 
awk -v OFS='\t' '{ print $1"_A", $3}' $output_dir/pop_file_${run_name}.txt > $output_dir/phased_pop_file_${run_name}.txt
awk -v OFS='\t' '{ print $1"_B", $3}' $output_dir/pop_file_${run_name}.txt >> $output_dir/phased_pop_file_${run_name}.txt
...
python ~/apps/twisst/twisst.py -t $output_dir/${run_name}.trees.gz -w $output_dir/${run_name}.weights.tsv.gz \
      -g DUIM -g CLAC -g LUIB -g TORM --method ${weight_method} --groupsFile $output_dir/phased_pop_file_${run_name}.txt
