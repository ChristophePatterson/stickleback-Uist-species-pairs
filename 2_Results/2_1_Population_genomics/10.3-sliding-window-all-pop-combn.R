setwd("C:/Users/mbzcp2/OneDrive - The University of Nottingham/Sticklebacks/Species Pairs M1/results/sliding_window/CLAC")

## CLAC resi vs all non CLAC anad
CLACr_v_nCLACa <- read.csv("sliding_window_w25kb_s5kb_m1_CLAC_resi_allanad.csv")
colnames(CLACr_v_nCLACa)[grep("Fst",colnames(CLACr_v_nCLACa))] <- "Fst"

## nonCLAC resi vs nonCLAC anad
nCLACr_v_nCLAMa <- read.csv("sliding_window_w25kb_s5kb_m1_nCLAC_resi_nCLAManad.csv")
colnames(nCLACr_v_nCLAMa)[grep("Fst",colnames(nCLACr_v_nCLAMa))] <- "Fst"

CLACr_v_nCLACr <- read.csv("sliding_window_w25kb_s5kb_m1_CLAC_resi_nCLACresi.csv")
colnames(CLACr_v_nCLACr)[grep("Fst",colnames(CLACr_v_nCLACr))] <- "Fst"

scaf <- read.table("C:/Users/mbzcp2/OneDrive - The University of Nottingham/Sticklebacks/Species Pairs M1/results/sliding_window/GCF_016920845.1_sequence_report.tsv", sep = "\t", header = T)
...
ggsave("Mirrored_CLACr_vs_nonCLACa_Fst.pdf", p, width = 10, height = 30)
...
ggsave("Mirrored_CLACr_vs_nonCLAMa_vs_nCLACr.pdf", p, width = 10, height = 30)


setwd("C:/Users/mbzcp2/OneDrive - The University of Nottingham/Sticklebacks/Species Pairs M1/results/sliding_window/All_Pop_comparison/")
## CLAC_CLAM
CLAC_CLAM <- read.csv("CLAC_CLAM/sliding_window_w25kb_s5kb_m1_CLAC_CLAM.csv")
...
## DUIN_DUIM
DUIN_DUIM <- read.csv("DUIM_DUIN/sliding_window_w25kb_s5kb_m1_DUIM_DUIN.csv")
...
## OBSE_OBSM
OBSE_OBSM <- read.csv("OBSE_OBSM/sliding_window_w25kb_s5kb_m1_OBSE_OBSM.csv")
...
## LUIB_LUIM
LUIB_LUIM <- read.csv("LUIB_LUIM/sliding_window_w25kb_s5kb_m1_LUIB_LUIM.csv")
...
ggsave("Mirrored_CLAC_CLAM_vs_LUIB_LUIM.pdf", p, width = 10, height = 30)

rbind(OBSE_OBSM, CLAC_CLAM, LUIB_LUIM)
...
ggsave("Fst_all_pop_comparisons.pdf", p, width = 10, height = 30)
