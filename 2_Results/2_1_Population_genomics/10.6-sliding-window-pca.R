...
p <- ggplot(pca.comp.df, aes(as.numeric(end), PCA1, col = Population, shape = Ecotype)) +
  geom_point() +
...
q <- ggplot(pca.comp.df, aes(as.numeric(end), PCA2, col = Population, shape = Ecotype)) +
  geom_point() +
  facet_grid(.~chr, scale = "free_x", space = "free_x") +
  theme_classic() +
  theme(legend.position = "top",panel.spacing = unit(0,'lines'),
...
p <- ggplot(pca.comp.df[pca.comp.df$chr=="IX"|pca.comp.df$chr=="I",], aes(as.numeric(end), PCA1, col = Population, shape = Ecotype)) +
  geom_point() +
  facet_wrap(~chr) +
  theme_classic() +
  theme(legend.position = "top",panel.spacing = unit(0,'lines'),
