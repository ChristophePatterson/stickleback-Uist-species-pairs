...
analysis_name=SFS_${SLURM_ARRAY_TASK_ID}_sigMig_${foldtype}
...
cp $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/SFS_sigMig_$foldtype/fastsimcoal2/*joint*.obs ./
...
echo "TDivResiEast@ $(awk '$1=="OBSE" {print NR-1}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_sigMig_uniq.txt) $(awk '$1=="DUIN" {print NR-1}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_sigMig_uniq.txt) 1 RESIZE2 0 0" >> $output_model/${analysis_name}.tpl
# CLAC merges into LUIB
echo "TDivResiWest@ $(awk '$1=="CLAC" {print NR-1}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_sigMig_uniq.txt) $(awk '$1=="LUIB" {print NR-1}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_sigMig_uniq.txt) 1 RESIZE1 0 0" >> $output_model/${analysis_name}.tpl
...
echo "1 TDivResiWest@ unif 1000 TDivResi@ output paramInRange">> $output_model/${analysis_name}.est
echo "1 TDivResiEast@ unif 1000 TDivResi@ output paramInRange">> $output_model/${analysis_name}.est

#  Complex parameters
echo "[COMPLEX PARAMETERS]" >> $output_model/${analysis_name}.est
echo "0 RESIZE1 = ResiWest$/LUIB$ hide" >> $output_model/${analysis_name}.est
echo "0 RESIZE2 = ResiWest$/DUIN$ hide" >> $output_model/${analysis_name}.est
echo "0 RESIZE3 = Resi$/ResiWest$ hide" >> $output_model/${analysis_name}.est
