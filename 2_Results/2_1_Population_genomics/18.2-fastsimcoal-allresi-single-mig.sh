...
foldtype=("unfolded")
# Analyse name
analysis_dir=allresi_singlemig_JSFS_r${randSNP}_Mono_${foldtype}_N12_bootstrap
analysis_name=allresi_singlemig_JSFS_r${randSNP}_Mono_${foldtype}_N12_A${SLURM_ARRAY_TASK_ID}
...
echo "TDivResiEast@ $(awk '$1=="OBSE" {print NR-1}' $output_dir/pop_uniq.txt) $(awk '$1=="DUIN" {print NR-1}' $output_dir/pop_uniq.txt) 1 RESIZE2 0 0" >> $output_dir/fsc_run/${analysis_name}.tpl
# CLAC merges into LUIB
echo "TDivResiWest@ $(awk '$1=="CLAC" {print NR-1}' $output_dir/pop_uniq.txt) $(awk '$1=="LUIB" {print NR-1}' $output_dir/pop_uniq.txt) 1 RESIZE1 0 0" >> $output_dir/fsc_run/${analysis_name}.tpl
# Resi west and east merge
echo "TDivResi@ $(awk '$1=="LUIB" {print NR-1}' $output_dir/pop_uniq.txt) $(awk '$1=="DUIN" {print NR-1}' $output_dir/pop_uniq.txt) 1 RESIZE3 0 0" >> $output_dir/fsc_run/${analysis_name}.tpl
# Resi and Migr merge into Ancestral
echo "TDivAncs@ $(awk '$1=="DUIN" {print NR-1}' $output_dir/pop_uniq.txt) $(awk '$1=="anad" {print NR-1}' $output_dir/pop_uniq.txt) 1 RESIZE4 0 0" >> $output_dir/fsc_run/${analysis_name}.tpl
...
## All popuulations
awk -v mxNPOP=$maxNPOP '{print "1 "$1"$ unif 1000 "mxNPOP" output"}' $output_dir/pop_uniq.txt >> $output_dir/fsc_run/${analysis_name}.est
...
echo "1 TDivResiWest@ unif 1000 TDivResi@ output paramInRange">> $output_dir/fsc_run/${analysis_name}.est
echo "1 TDivResiEast@ unif 1000 TDivResi@ output paramInRange">> $output_dir/fsc_run/${analysis_name}.est
...
echo "0 RESIZE2 = ResiWest$/DUIN$ hide" >> $output_dir/fsc_run/${analysis_name}.est
