...
echo "8 samples to simulate :" >> $output_model/${analysis_name}.tpl
...
awk '{print $1"$"}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_all_uniq.txt >> $output_model/${analysis_name}.tpl
echo "//Samples sizes and samples age" >> $output_model/${analysis_name}.tpl
awk '{print $2}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/SFS_${SLURM_ARRAY_TASK_ID}_all_best_proj_unfolded.txt >> $output_model/${analysis_name}.tpl
echo "//Growth rates: negative growth implies population expansion" >> $output_model/${analysis_name}.tpl
awk '{print "0"}' $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_all_uniq.txt >> $output_model/${analysis_name}.tpl
...
## Plot maxPar file
Rscript ~/code/Github/stickleback-Uist-species-pairs/Helper_scripts/ParFileViewer.R ${analysis_name}_maxL.par $output_dir/SFS/SFS_${SLURM_ARRAY_TASK_ID}/pop_all_uniq.txt
